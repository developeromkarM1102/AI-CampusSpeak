...
const AdminRegisterSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://ai-campusspeak.onrender.com/api/auth/admin/register",
  ...
  return (
    <div className="bg-[url('/Login2.webp')] bg-cover bg-center w-full min-h-screen flex items-center justify-center px-4 py-8 md:py-20">
      <div className='flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-20 w-full max-w-7xl mt-5'>
          ...
          >
            ...
            <p className='text-black text-sm md:text-base text-center mb-6'>
              ...
              <span className='inline-block ml-3 bg-blue-500 px-4 py-2 rounded-2xl'>
                <a href="/admin/login" className='cursor-pointer text-white hover:text-gray-200'>
        ...
        <img
          src="/Login.webp"
          className="w-full max-w-md lg:max-w-xl rounded-2xl object-cover hidden md:block mt-20"
          alt="login"
        />
      </div>
    </div>
